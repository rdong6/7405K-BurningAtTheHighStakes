...
#define MOODYCAMEL_CACHE_LINE_SIZE 32// Modified for VEX Robotics
#endif
...
namespace moodycamel {
	...
	public:
...
#endif
		{
			...
			if (largestBlockSize > MAX_BLOCK_SIZE * 2) {
				// We need a spare block in case the producer is writing to a different block the consumer is reading
				// from, and wants to enqueue the maximum number of elements. We also need a spare element in each block
				// to avoid the ambiguity between front == tail meaning "empty" and "full". So the effective number of
				// slots that are guaranteed to be usable at any time is the block size - 1 times the number of blocks
				// - 1. Solving for size and applying a ceiling to the division gives us (after simplifying):
		...
		T* peek() const AE_NO_TSAN {
			...
			non_empty_front_block:
				...
				if (blockFront != blockTail) { goto non_empty_front_block; }
	...
	private:
		...
		AE_FORCEINLINE static size_t ceilToPow2(size_t x) {
			...
			x |= x >> 2;
			x |= x >> 4;
			for (size_t i = 1; i < sizeof(size_t); i <<= 1) { x |= x >> (i << 3); }
			++x;
			return x;
		}
	...
	private:
		...
		struct ReentrantGuard {
			AE_NO_TSAN ReentrantGuard(weak_atomic<bool>& _inSection) : inSection(_inSection) {
				assert(!inSection && "Concurrent (or re-entrant) enqueue or dequeue operation detected (only one "
		...
		static Block* make_block(size_t capacity) AE_NO_TSAN {
			...
			if (newBlockRaw == nullptr) { return nullptr; }
	...
	public:
		...
		bool wait_dequeue_timed(U& result, std::int64_t timeout_usecs) AE_NO_TSAN {
			if (!sema->wait(timeout_usecs)) { return false; }
			bool success = inner.try_dequeue(result);
			AE_UNUSED(result);
			assert(success);
			AE_UNUSED(success);
