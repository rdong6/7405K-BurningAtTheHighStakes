...
namespace moodycamel {
	...
	namespace spsc_sema {
		...
		public:
			AE_NO_TSAN Semaphore(int initialCount = 0) : m_sema() {
				assert(initialCount >= 0);
				m_sema = sem_create(static_cast<uint32_t>(~0ull), static_cast<uint32_t>(initialCount));
			...
			bool wait() AE_NO_TSAN {
				return sem_wait(m_sema, (uint32_t) 0xffffffffUL) == 1;
			...
			bool try_wait() AE_NO_TSAN {
				...
				// if (xPortIsInsideInterrupt()) return xSemaphoreTakeFromISR(m_sema, NULL) == pdTRUE;

				return sem_wait(m_sema, 0) == 1;
				// return xSemaphoreTake(m_sema, 0) == pdTRUE;
			}

			bool timed_wait(std::uint64_t usecs) AE_NO_TSAN {
				std::uint64_t msecs = usecs / 1000;
				uint32_t ticks = static_cast<uint32_t>(msecs / ((uint32_t) 1000 / ((uint32_t) 1000)));
				if (ticks == 0) return try_wait();
				return sem_wait(m_sema, ticks) == 1;
			...
			void signal() AE_NO_TSAN {
				...
				// if (xPortIsInsideInterrupt()) rc = xSemaphoreGiveFromISR(m_sema, NULL);
				// else
