...
#define MOODYCAMEL_CACHE_LINE_SIZE 32
#endif
...
namespace moodycamel {
	...
	public:
		explicit BlockingReaderWriterCircularBuffer(std::size_t capacity)
			...
			capacity |= capacity >> 2;
			capacity |= capacity >> 4;
			for (std::size_t i = 1; i < sizeof(std::size_t); i <<= 1) capacity |= capacity >> (i << 3);
			mask = capacity++;
		...
		bool try_enqueue(T const& item) {
			if (!slots_->tryWait()) return false;
			inner_enqueue(item);
			return true;
		}
		...
		bool wait_enqueue_timed(T const& item, std::int64_t timeout_usecs) {
			if (!slots_->wait(timeout_usecs)) return false;
			inner_enqueue(item);
			return true;
		}
		...
		bool wait_dequeue_timed(U& item, std::int64_t timeout_usecs) {
			if (!items->wait(timeout_usecs)) return false;
			inner_dequeue(item);
			return true;
		}
		...
		// Thread-safe when called by consumer thread.
		inline T* peek() {
			if (!items->availableApprox()) return nullptr;
			return inner_peek();
		}
